...
class VersionSet::Builder {
 ...
 public:
  ...
  void Apply(const VersionEdit* edit) {
    ...
    // 首先更新compaction_pointer_，这个直接在VersionSet里更新，因为这个的信息只有一份
...
Compaction* VersionSet::PickCompaction() {
  ...
  // 如果c->inputs_里的文件是Level 0，那么同一Level其它文件也有可能有重叠，
  // 则找到其它重叠的文件
  ...
  // 找到上一层Level和选定的Level有重叠的文件，
  // 这样就找到了两层需要Compaction的文件
  SetupOtherInputs(c);
